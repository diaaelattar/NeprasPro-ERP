// ════════════════════════════════════════════════════════════════
//  Report Definition: سجل الطلاب الموقوف قيدهم (مثل سجل 41)
// ════════════════════════════════════════════════════════════════
                ...
                {pageStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id || idx}>
                      ...
                      <td className="cell-sm" style={{ fontWeight: 800, color: '#d97706' }}>موقوف قيده</td>
...
const suspendedRegister = {
  ...
  excelEndpoint: (f) =>
    ...
    `&sectionId=${f.sectionId || ''}&stageId=${f.stageId || ''}&status=suspended&genderOrder=${f.genderOrder || 'none'}`,

  excelFileName: (f, meta) =>
    `سجل_الطلاب_الموقوف_قيدهم_${meta.selectedGrade?.grade_name_ar || ''}_${meta.selectedYear?.year_label?.replace('/', '_') || ''}.xlsx`,
