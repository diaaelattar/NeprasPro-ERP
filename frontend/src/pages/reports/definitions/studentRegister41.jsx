                ...
                {pageStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id || idx}>
                      ...
                      <td className="cell-sm" style={{ fontWeight: 800, color: '#15803d' }}>مستجد (منقول)</td>
...
const studentRegister41 = {
  ...
  // URL لتصدير Excel من الـ backend للمستجدين فقط
  excelEndpoint: (f) =>
    `/api/students/export/excel?gradeId=${f.gradeId}&academicYearId=${f.academicYearId}` +
    `&sectionId=${f.sectionId || ''}&stageId=${f.stageId || ''}&status=promoted&genderOrder=${f.genderOrder || 'none'}`,

  excelFileName: (f, meta) =>
    `سجل_41_مستجدين_${meta.selectedGrade?.grade_name_ar || ''}_${meta.selectedYear?.year_label?.replace('/', '_') || ''}.xlsx`,

  // دالة تحميل البيانات للمعاينة للمستجدين فقط
  buildQuery: (f) => {
    const q = new URLSearchParams({
      gradeId: f.gradeId,
      academicYearId: f.academicYearId,
      limit: '10000',
      status: 'promoted', // فرز أوتوماتيكي للطلاب المستجدين (منقول) فقط
    });
