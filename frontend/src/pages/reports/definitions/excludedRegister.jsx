// ════════════════════════════════════════════════════════════════
//  Report Definition: سجل الطلاب المستبعدين (مثل سجل 41)
// ════════════════════════════════════════════════════════════════
...
/* ── Preview Component ─────────────────────────────────────────── */
function ExcludedRegisterPreview({ students = [], meta = {}, schoolInfo = {} }) {
        ...
        <div key={pageIndex} className="printable-page-block">
          ...
          <div className="report-official-header">
            ...
            <div className="header-col-center">
              <h2 className="report-title-main" style={{ color: '#991b1b' }}>
                سجل الطلاب المستبعدين - الصف {selectedGrade?.grade_name_ar || 'جميع الصفوف'}
              </h2>
              <div className="report-subtitle-meta">
                للعام الدراسي {selectedYear?.year_label || '...............'} | إجمالي المستبعدين: {students.length} طالب
              </div>
            </div>
                ...
                {pageStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id || idx}>
                      ...
                      <td className="cell-sm" style={{ fontWeight: 800, color: '#991b1b' }}>مستبعد</td>
...
const excludedRegister = {
  ...
  excelEndpoint: (f) =>
    ...
    `&sectionId=${f.sectionId || ''}&stageId=${f.stageId || ''}&status=excluded&genderOrder=${f.genderOrder || 'none'}`,

  excelFileName: (f, meta) =>
    `سجل_الطلاب_المستبعدين_${meta.selectedGrade?.grade_name_ar || ''}_${meta.selectedYear?.year_label?.replace('/', '_') || ''}.xlsx`,
...
export default excludedRegister;
