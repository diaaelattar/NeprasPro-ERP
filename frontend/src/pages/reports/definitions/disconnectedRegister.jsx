// ════════════════════════════════════════════════════════════════
//  Report Definition: سجل الطلاب المنقطعين عن الدراسة
// ════════════════════════════════════════════════════════════════
...
/* ── Preview Component ─────────────────────────────────────────── */
function DisconnectedRegisterPreview({ students = [], meta = {}, schoolInfo = {} }) {
        ...
        <div key={pageIndex} className="printable-page-block" style={{ padding: '10px 14px', boxSizing: 'border-box' }}>
          ...
          <div className="report-official-header" style={{ marginBottom: 8, paddingBottom: 6, borderBottom: '1.5px solid #000', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <div className="header-col-center" style={{ textAlign: 'center', flex: 1 }}>
              <h2 className="report-title-main" style={{ fontSize: 15.5, fontWeight: 900, margin: 0, textDecoration: 'underline', color: '#b91c1c' }}>
                سجل المنقطعين عن الدراسة - {selectedGrade?.grade_name_ar || 'جميع الصفوف'}
              </h2>
              <div className="report-subtitle-meta" style={{ fontSize: 11, fontWeight: 700, color: '#334155', marginTop: 2 }}>
                للعام الدراسي {selectedYear?.year_label || '...............'} | إجمالي المنقطعين: <strong>{students.length}</strong> طالب
              </div>
            </div>
            ...
            <table className="register-table" dir="rtl" style={{ width: '100%', borderCollapse: 'collapse', border: '1.5px solid #000', fontSize: 9.5, textAlign: 'center', tableLayout: 'fixed' }}>
              <thead>
                <tr style={{ background: '#fee2e2', color: '#991b1b', fontWeight: 800 }}>
                  ...
                  <th colSpan="2" style={{ border: '1px solid #000', padding: '2px 4px', width: 195, background: '#fecaca' }}>ولي الأمر</th>
                ...
                <tr style={{ background: '#fef2f2', color: '#991b1b', fontWeight: 800, fontSize: 9 }}>
              ...
              <tbody>
                {pageStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id || idx} style={{ background: idx % 2 === 1 ? '#fff5f5' : '#fff' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', border: '1.5px solid #000', fontSize: 9.5, textAlign: 'center' }}>
                <thead>
                  <tr style={{ background: '#fee2e2', fontWeight: 800 }}>
                    ...
                    <th colSpan="3" style={{ border: '1px solid #000', padding: '3px', background: '#fecaca' }}>الديانة المسلمة</th>
                    <th colSpan="3" style={{ border: '1px solid #000', padding: '3px', background: '#fecaca' }}>الديانة المسيحية</th>
...
const disconnectedRegister = {
  id:          'disconnected_register',
  name:        'سجل الطلاب المنقطعين',
  desc:        'سجل تفصيلي بالطلاب المنقطعين عن الدراسة مع بيانات ولي الأمر وعناوينهم',
  ...
  excelFileName: (filters, meta) => {
    ...
    return `سجل_المنقطعين_${gradeName}.xlsx`;
  },

  PreviewComponent: DisconnectedRegisterPreview
};

export default disconnectedRegister;
