// ════════════════════════════════════════════════════════════════
//  Report Definition: سجل الطلاب المنقطعين عن الدراسة (مثل سجل 41)
// ════════════════════════════════════════════════════════════════
                ...
                {pageStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id || idx}>
                      ...
                      <td className="cell-sm" style={{ fontWeight: 800, color: '#dc2626' }}>منقطع عن الدراسة</td>
...
const disconnectedRegister = {
  ...
  excelEndpoint: (f) =>
    ...
    `&sectionId=${f.sectionId || ''}&stageId=${f.stageId || ''}&status=disconnected&genderOrder=${f.genderOrder || 'none'}`,

  excelFileName: (f, meta) =>
    `سجل_الطلاب_المنقطعين_${meta.selectedGrade?.grade_name_ar || ''}_${meta.selectedYear?.year_label?.replace('/', '_') || ''}.xlsx`,
