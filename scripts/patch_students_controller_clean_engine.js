...
// 1. New clean exportExcelTemplate
...
// 2. New clean exportClassListExcel
...
// 3. New clean exportFullClassListExcel
const newExportFullClassListExcel = `const exportFullClassListExcel = async (req, res) => {
  if (!db.isConfigured()) return res.status(400).json({ success: false, error: 'قاعدة البيانات غير مهيأة.' });
  try {
    const sqliteDb = db.getSQLiteDb();
    const { search, sectionId, stageId, gradeId, classId, status, academicYearId, genderOrder } = req.query;
...
// Replace exportExcelTemplate
const oldTemplateStart = content.indexOf('const exportExcelTemplate = async (req, res) => {');
const oldTemplateEnd = content.indexOf('const injectXmlCellText = (sheetXml, cellRef, text) => {');
if (oldTemplateStart !== -1 && oldTemplateEnd !== -1) {
  content = content.substring(0, oldTemplateStart) + newExportExcelTemplate + '\n\n' + content.substring(oldTemplateEnd);
  console.log('✅ exportExcelTemplate replaced cleanly!');
}

// Replace exportClassListExcel
const oldClassListStart = content.indexOf('const exportClassListExcel = async (req, res) => {');
const oldClassListEnd = content.indexOf('const exportFullClassListExcel = async (req, res) => {');
if (oldClassListStart !== -1 && oldClassListEnd !== -1) {
  content = content.substring(0, oldClassListStart) + newClassListModes + '\n}\n\n' + content.substring(oldClassListEnd);
  console.log('✅ exportClassListExcel replaced cleanly!');
}

// Replace exportFullClassListExcel
const oldFullStart = content.indexOf('const exportFullClassListExcel = async (req, res) => {');
const oldFullEnd = content.indexOf('const exportTransfersExcel = async (req, res) => {');
if (oldFullStart !== -1 && oldFullEnd !== -1) {
  content = content.substring(0, oldFullStart) + newExportFullClassListExcel + '\n\n' + content.substring(oldFullEnd);
  console.log('✅ exportFullClassListExcel replaced cleanly!');
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('✅ students.controller.js patched with clean standalone engine!');
