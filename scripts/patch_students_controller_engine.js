...
// Replace any remaining require('./excelSanitizer') if exists
content = content.replace(/const \{ sanitizeExcelBuffer \} = require\('\.\/excelSanitizer'\);\n?/g, '');

// Ensure excelReportEngine is required at top of file
if (!content.includes("excelReportEngine")) {
  content = `const excelReportEngine = require('../../services/excelReportEngine');\n` + content;
}

// 1. Rewrite exportExcelTemplate to use excelReportEngine
const oldExportExcelTemplateStart = `const exportExcelTemplate = async (req, res) => {`;
const oldExportExcelTemplateEnd = `    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', \`attachment; filename=register_report.xlsx\`);
    
    const buffer = await wb.xlsx.writeBuffer();
    return res.send(await sanitizeExcelBuffer(buffer));
  } catch (err) {
...
// Replace exportExcelTemplate in content
const templateIdxStart = content.indexOf(oldExportExcelTemplateStart);
const templateIdxEnd = content.indexOf('return res.status(500).json({ success: false, error: \'فشل تصدير ملف الإكسيل الرئيسي.\' });\n  }\n};');

if (templateIdxStart !== -1 && templateIdxEnd !== -1) {
  const fullOldBlock = content.substring(templateIdxStart, templateIdxEnd + 'return res.status(500).json({ success: false, error: \'فشل تصدير ملف الإكسيل الرئيسي.\' });\n  }\n};'.length);
  content = content.replace(fullOldBlock, newExportExcelTemplate);
  console.log('✅ exportExcelTemplate rewritten using excelReportEngine!');
} else {
  console.log('⚠️ Could not match exportExcelTemplate block index.');
}

// 2. Rewrite exportClassListExcel to use excelReportEngine for landscape and portrait modes
const oldExportClassListExcelStart = `const exportClassListExcel = async (req, res) => {`;
const oldExportClassListExcelEnd = `    res.setHeader('Content-Type', 'application/vnd.ms-excel.sheet.macroEnabled.12');
    res.setHeader('Content-Disposition', \`attachment; filename="primary_portrait_\${safeClassName}.xlsm"; filename*=UTF-8''\${arabicFileName}\`);
    return res.send(await sanitizeExcelBuffer(finalBuffer));
  }`;

const newClassListModes = `const exportClassListExcel = async (req, res) => {
  if (!db.isConfigured()) return res.status(400).json({ success: false, error: 'قاعدة البيانات غير مهيأة.' });
  if (req.query.mode === 'full' || req.query.type === 'full') {
    return exportFullClassListExcel(req, res);
  }

  if (req.query.mode === 'primary_landscape' || req.query.mode === 'primary_portrait') {
    const sqliteDb = db.getSQLiteDb();
...
const classListIdxStart = content.indexOf(oldExportClassListExcelStart);
const classListIdxEnd = content.indexOf(oldExportClassListExcelEnd);

if (classListIdxStart !== -1 && classListIdxEnd !== -1) {
  const fullOldClassListBlock = content.substring(classListIdxStart, classListIdxEnd + oldExportClassListExcelEnd.length);
  content = content.replace(fullOldClassListBlock, newClassListModes);
  console.log('✅ exportClassListExcel rewritten using excelReportEngine!');
} else {
  console.log('⚠️ Could not match exportClassListExcel block index.');
}

// 3. Rewrite exportFullClassListExcel to use excelReportEngine
const oldExportFullStart = `const exportFullClassListExcel = async (req, res) => {`;
const oldExportFullEnd = `    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
const newExportFull = `const exportFullClassListExcel = async (req, res) => {
  if (!db.isConfigured()) return res.status(400).json({ success: false, error: 'قاعدة البيانات غير مهيأة.' });
  try {
    const sqliteDb = db.getSQLiteDb();
    const { search, sectionId, stageId, gradeId, classId, status, academicYearId, genderOrder } = req.query;
...
const fullIdxStart = content.indexOf(oldExportFullStart);
const fullIdxEnd = content.indexOf(oldExportFullEnd);

if (fullIdxStart !== -1 && fullIdxEnd !== -1) {
  const fullOldFullBlock = content.substring(fullIdxStart, fullIdxEnd + oldExportFullEnd.length);
  content = content.replace(fullOldFullBlock, newExportFull);
  console.log('✅ exportFullClassListExcel rewritten using excelReportEngine!');
} else {
  console.log('⚠️ Could not match exportFullClassListExcel block index.');
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('✅ students.controller.js fully updated with excelReportEngine!');
